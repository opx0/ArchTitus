...
if [[ "$PARTITION_MODE" == "CUSTOM" ]]; then
    # CUSTOM MODE: Skip wiping and partitioning
    echo -ne "
-------------------------------------------------------------------------
                    Using Existing Partitions
-------------------------------------------------------------------------
"
    partition2=${PARTITION_EFI}
    partition3=${PARTITION_ROOT}
    echo "Using EFI Partition: $partition2"
    echo "Using ROOT Partition: $partition3"

else
    # AUTO MODE: Wipe and Repartition
    # disk prep
    ...
    # Define partition variables for Auto mode
    if [[ "${DISK}" =~ "nvme" ]]; then
        partition2=${DISK}p2
        partition3=${DISK}p3
    else
...
# Format Helper Logic
# If Custom Mode, DO NOT Format EFI, only ROOT.
# If Auto Mode, Format both.

if [[ "${FS}" == "btrfs" ]]; then
    if [[ "$PARTITION_MODE" != "CUSTOM" ]]; then mkfs.vfat -F32 -n "EFIBOOT" ${partition2}; fi
