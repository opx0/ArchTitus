...
if [[ ! $DESKTOP_ENV == server ]]; then
    ...
    # sed matches partial string (e.g. "MINIMAL" matches "--END OF MINIMAL INSTALL--")
    # grep -v filters out the delimiter line if we are doing a FULL install
    pkgs=$(sed -n "/$INSTALL_TYPE/q;p" "$HOME/ArchTitus/pkg-files/pacman-pkgs.txt" | grep -v "^--END OF MINIMAL INSTALL--")
