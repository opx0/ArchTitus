...
if [[ ! $DESKTOP_ENV == server ]]; then
  # Batch install packages to reduce pacman overhead
  packages=()
  while read -r line; do
    # Skip the marker and any empty lines
    if [[ ${line} == '--END OF MINIMAL INSTALL--' ]] || [[ -z "${line}" ]]; then
      continue
    fi
    packages+=("${line}")
  done < <(sed -n '/'$INSTALL_TYPE'/q;p' $HOME/ArchTitus/pkg-files/pacman-pkgs.txt)
